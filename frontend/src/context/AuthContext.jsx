...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    const t = localStorage.getItem('token_caso3')
    const u = localStorage.getItem('usuario_caso3')
  ...
  const login = (data) => {
    ...
    localStorage.setItem('token_caso3', data.token)
    localStorage.setItem('usuario_caso3', JSON.stringify(u))
  }

  const logout = () => {
    setToken(null); setUsuario(null)
    localStorage.removeItem('token_caso3')
    localStorage.removeItem('usuario_caso3')
  }
